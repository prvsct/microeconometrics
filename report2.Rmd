...
# Tem que fazer um desse pra cada variável (density_test_ifh, density_test_water_bill, density_test_electricity_bill)

# exemplo com ifh

density_test_ifh <- rddensity::rddensity(X = database$ifh, c = 55)

summary(density_test_ifh)

# A gente queria não rejeitar H_0, o pvalor tá no objeto density_test_ifh$test[4]
ifh_density_pvalue <- density_test_ifh$test[4]

# E daí faz o plot:
density_test_ifh_plot <- rddensity::rdplotdensity(rdd = density_test_ifh, X = database$ifh, title = "Density of the ifh variable")
```
...
covariate_results <- data.frame(
  ...
  "robust_pvalue" = numeric(7),
  "robust_ci_lower" = numeric(7),
  "robust_ci_upper" = numeric(7)
)
...
```{r eval=FALSE, include=FALSE}
# Exemplo com woman
# Ainda tem que usar o database_filtered

covariate_balance_test_ifh <- rdrobust::rdrobust(y = database_filtered$woman,
                                                 x = database_filtered$ifh,
                                                 cluster = database_filtered$hh_id,
                                                 c = 55)

# Ele vai dar aviso de mass point provavelmente porque o ifh teve adjusting
# A gente queria não rejeitar H_0 também. O pvalor robusto tá no objeto covariate_balance_test_ifh$pv[3]
covariate_balace_ifh_pvalue <- covariate_balance_test_ifh$pv[3]

# Daí faz o plot do mesmo jeito de antes
covariate_balance_ifh_plot <- rdrobust::rdplot(y = database_filtered$woman,
                                               x = database_filtered$ifh,
                                               c = 55,
                                               p = 4,
                                               title = "Density of the woman covariate")
...
```{r eval=FALSE, include=FALSE}
# Exemplo com medicines
# De novo precisa ser o database filtered

medicines_sharp <- rdrobust::rdrobust(y = database_filtered$medicines,
                                  x = database_filtered$ifh,
                                  cluster = database_filtered$hh_id,
                                  c = 55)

# De novo vai dar aviso de mass point
# O coeficiente, o pvalor robusto e os intervalos de confianca robustos estao nos objetos medicines_sharp$coef[1], medicines_sharp$pv[3], medicines_sharp$ci[3,1] (lower) e medicines_sharp$ci[3,2] (upper)

medicines_sharp_coef <- medicines_sharp$coef[1]
medicines_sharp_robust_pvalue <- medicines_sharp$pv[3]
medicines_sharp_ci_lower <- medicines_sharp$ci[3,1]
medicines_sharp_ci_upper <- medicines_sharp$ci[3,2]

# Plot
medicines_sharp_plt <- rdrobust::rdplot(y = database_filtered$medicines,
                                   x = database_filtered$ifh,
                                   c = 55,
                                   p = 4,
                                   title = "Plot of the medicines outcome variable")
```

As we can see, in both specifications, the estimated coefficients for all outcomes are not significant at any reasonable level.
...
To run a fuzzy RD, we have to construct a ``treatment`` variable that is equal to $1$ when the household is fully eligible, and $0$ otherwise. Then we set ``fuzzy = treatment``. Since the graphs are the same as the sharp case, I have omitted them here.
...
medicines_fuzzy <- rdrobust::rdrobust(y = database_fuzzy$medicines,
                                      x = database_fuzzy$ifh,
                                      fuzzy = database_fuzzy$treatment,
                                      cluster = database_fuzzy$hh_id,
                                      c = 55)

# De novo vai dar aviso de mass point
# O coeficiente, o pvalor robusto e os intervalos de confianca robustos estao nos objetos medicines_fuzzy$coef[1], medicines_fuzzy$pv[3], medicines_fuzzy$ci[3,1] (lower) e medicines_fuzzy$ci[3,2] (upper)

medicines_fuzzy_coef <- medicines_fuzzy$coef[1]
medicines_fuzzy_robust_pvalue <- medicines_fuzzy$pv[3]
medicines_fuzzy_ci_lower <- medicines_fuzzy$ci[3,1]
medicines_fuzzy_ci_upper <- medicines_fuzzy$ci[3,2]
